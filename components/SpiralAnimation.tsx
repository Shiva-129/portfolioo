...
class AnimationController {
    ...
    private setupTimeline() {
        this.timeline
            .to(this, {
                time: 1,
                duration: 10, // Slowed down to 10 seconds
                ease: "none",
...
class Star {
    ...
    private finalScale: number // Final Scale

    constructor(cameraZ: number, cameraTravelDistance: number) {
        this.angle = Math.random() * Math.PI * 2
        this.distance = 30 * Math.random() + 15
        this.rotationDirection = Math.random() > 0.5 ? 1 : -1
        this.expansionRate = 1.2 + Math.random() * 0.8 // Increase expansion rate
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            // Mix different easing effects
            let easing;
            if (displacementProgress < 0.3) {
                // Start phase: Linear and Quadratic
...
export function SpiralAnimation({ onComplete }: SpiralAnimationProps) {
    ...
    // Store onComplete in a ref to avoid re-triggering useEffect
    ...
    useEffect(() => {
        ...
        // Pass a wrapper function that calls the ref
        ...
        // Removed onComplete from dependency array to prevent restart on callback change
    }, [dimensions])
